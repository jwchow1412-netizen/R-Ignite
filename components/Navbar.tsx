  ...
  const renderLink = (link: { name: string; href: string }) => {
    ...
    return (
      <Link
        key={link.name}
        href={link.href}
        className={`px-3 py-2 text-sm font-semibold transition-colors rounded-lg ${active
        ...
        <nav className="hidden items-center gap-2 lg:flex">
          ...
          <div className="relative">
            <button
              ...
              className={`px-3 py-2 text-sm font-semibold rounded-lg inline-flex items-center gap-1 transition-colors ${pathname.startsWith("/people")
                ? "text-white bg-[rgba(212,100,118,0.15)] border border-[rgba(212,100,118,0.35)]"
                : "text-[rgba(248,244,246,0.82)] hover:text-white hover:bg-[rgba(255,255,255,0.06)]"
                }`}
          ...
          <div className="flex items-center gap-3">
            {user ? (
              <>
                <Button asChild variant="outline" className="border-[rgba(212,100,118,0.35)] bg-[rgba(212,100,118,0.1)] hover:bg-[rgba(212,100,118,0.2)] text-white">
                  <Link href="/rewards">
                    Rewards Portal ✨
                  </Link>
                </Button>
                {isAdmin ? (
                  <Button asChild className="bg-gradient-to-br from-[#7c1c39] to-[#d46476] text-white shadow-[0_14px_30px_rgba(212,100,118,0.25)]">
              ...
              {user ? (
                <>
                  ...
                  {isAdmin ? (
                    <Button
                      asChild
                      className="w-full max-w-[150px] bg-gradient-to-br from-[#7c1c39] to-[#d46476] text-white"
                      onClick={() => setMobileOpen(false)}
                    >
                      <Link href="/rewards/admin">
                        Admin Panel
